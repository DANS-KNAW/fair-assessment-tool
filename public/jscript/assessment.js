...
function get_value(element) {
  return checked(element)
    ? parseFloat(document.getElementById(element).value)
    : 0.0;
}
...
function write_to_modal(title, contents, modal_nr = 1) {
  ...
  $(`#modal-${modal_nr}`).attr("aria-label", title);
  ...
  if (question != null) {
    ...
    for (let letter of letters) {
      ...
      document.getElementById(("willingness-" + letter).toLowerCase()).src =
        "images/print/yellow/" + letter + "_" + willingness_percent + ".jpg";
    }
  }
}
  ...
  for (let [letter, questions] of fields) {
    for (let i = 0; i < questions.length; i++) {
      ...
      if (!answered && !excluded(question)) {
        ...
        document.getElementById(question_key).style.color = "#d60000";
        unanswered = true;
      }
...
function show_results() {
  ...
  if (get_awareness_score() < number_fair_questions) {
    ...
    hide_element("guidance-texts");
  }
  if (window.print) {
    show_element("print-button");
  }
}
...
var social_media = function (media) {
  ...
  switch (media) {
    ...
    case "linkedin":
      url =
        "https://www.linkedin.com/shareArticle?mini=true&url=https://fairaware.dans.knaw.nl";
      break;
    case "whatsapp":
  ...
  text =
    media == "linkedin"
      ? ""
      : "I just used #FAIRAwareTool to assess and increase my knowledge on the #FAIR data principles! Try it out for yourself here: https://fairaware.dans.knaw.nl/";
  if (media == "twitter")
    text = text + "\n\n@DANS_knaw_nwo | @FAIRsFAIR_eu | #FAIRAware";
  dimensions =
    media == "whatsapp" ? "width=1200,height=800" : "width=600,height=600";
